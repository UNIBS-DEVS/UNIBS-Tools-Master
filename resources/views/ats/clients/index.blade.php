...
@section('content')
    ...
    <div class="d-flex justify-content-between align-items-center mb-3">
        <h3>ATS Clients Master</h3>

        <a href="{{ route('ats.clients.create') }}" class="btn btn-primary">
            ...
            <tbody>
                @forelse ($clients as $client)
                    <tr>
                        ...
                        <td>
                            <a href="{{ route('ats.clients.show', $client->id) }}" class="btn btn-outline-info btn-sm">
                                <i class="fa fa-eye"></i>
                            </a>

                            <a href="{{ route('ats.clients.edit', $client->id) }}" class="btn btn-outline-warning btn-sm">
                                <i class="fa-solid fa-pen"></i>
                            </a>

                            <a href="{{ route('ats.clientsSysConfigs.create', $client->id) }}"
                                class="btn btn-outline-secondary btn-sm" title="Configuration">
                                <i class="fa-solid fa-gear"></i>
                            </a>

                            <form action="{{ route('ats.clients.destroy', $client->id) }}" method="POST" class="d-inline">
                                @csrf
                                @method('DELETE')
