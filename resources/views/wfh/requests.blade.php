...
@section('title', 'Manager WFH Approvals | Unibs Tools')
    ...
    <div class="container mt-4">
        ...
        <div class="card shadow-sm border-0">
            ...
            <div class="card-header bg-white d-flex justify-content-between align-items-center">
                ...
                <form method="GET" action="{{ route('manager.wfh.requests') }}">
            ...
            <div class="card-body table-responsive">
                ...
                <table id="wfhTable" class="table table-bordered table-hover align-middle bg-white">
                    ...
                    <thead class="table-dark">
                        ...
                        <tr class="table-light filter-row">
                            <th><input type="text" class="form-control form-control-sm wfh-filter" data-col="0"></th>
                            <th><input type="text" class="form-control form-control-sm wfh-filter" data-col="1"></th>
                            <th><input type="text" class="form-control form-control-sm wfh-filter" data-col="2"></th>
                            <th><input type="text" class="form-control form-control-sm wfh-filter" data-col="3"></th>
                            <th></th>
                            <th><input type="text" class="form-control form-control-sm wfh-filter" data-col="5"></th>
                            <th></th>
                            <th><input type="text" class="form-control form-control-sm wfh-filter" data-col="7"></th>
                                ...
                                <td class="text-center">
                                    @if(strtolower($request->status) == 'submitted')
                                        <form id="process-form-{{ $request->id }}" method="POST"
                                            action="{{ route('manager.wfh.process', $request->id) }}" class="d-inline-flex gap-1">
                                            @csrf
...
@push('scripts')
    <script>
        $(document).ready(function () {
            $('.wfh-filter').on('keyup change', function () {
                $('#wfhTable tbody tr').each(function () {
                    let show = true;

                    let employee = $(this).find('td:eq(0)').text().toLowerCase();
                    let employeeFilter = $('.wfh-filter[data-col="0"]').val().toLowerCase();

                    let date = $(this).find('td:eq(1)').text().toLowerCase();
                    let dateFilter = $('.wfh-filter[data-col="1"]').val().toLowerCase();

                    let type = $(this).find('td:eq(2)').text().toLowerCase();
                    let typeFilter = $('.wfh-filter[data-col="2"]').val().toLowerCase();

                    let reason = $(this).find('td:eq(3)').text().toLowerCase();
                    let reasonFilter = $('.wfh-filter[data-col="3"]').val().toLowerCase();

                    let remarks = $(this).find('td:eq(5)').text().toLowerCase();
                    let remarksFilter = $('.wfh-filter[data-col="5"]').val().toLowerCase();

                    let actionTime = $(this).find('td:eq(7)').text().toLowerCase();
                    let actionTimeFilter = $('.wfh-filter[data-col="7"]').val().toLowerCase();
