...
@section('title', 'Update Repair Request | Unibs Tools')
                ...
                <div class="card shadow-sm border-0 mb-4">
                    ...
                    <div class="card-header bg-white border-bottom py-3">
                        <h5 class="mb-0 fw-semibold text-dark">
                            ...
                            Update / Complete Asset Repair
                        </h5>
                    </div>

                    {{-- Body --}}
                    <div class="card-body p-4">
                        <form method="POST" action="{{ route('asset-repairs.update', $repair->id) }}">
                            @csrf
                            @method('PUT')
                            ...
                            <div class="mb-3">
                                ...
                                <div class="input-group">
                                    ...
                                    @php
                                        $preselectedAsset = $assets->firstWhere('id', $repair->asset_id);
                                    @endphp
                                    <select class="form-select" disabled>
                                        ...
                                        @else
                                            @php
                                                $fallbackAsset = \App\Models\AssetMaster::find($repair->asset_id);
                                            @endphp
                                            @if ($fallbackAsset)
                                ...
                                <div class="input-group">
                                    ...
                                    <select name="vendor_id" class="form-select @error('vendor_id') is-invalid @enderror">
                                        ...
                                        @foreach ($vendors as $vendor)
                                            <option value="{{ $vendor->id }}" {{ old('vendor_id', $repair->vendor_id) == $vendor->id ? 'selected' : '' }}>
                                                {{ $vendor->vendor_name }}
                                            </option>
                                        @endforeach
                                    </select>
                            ...
                            <div class="mb-3">
                                ...
                                <textarea name="issue_description" class="form-control @error('issue_description') is-invalid @enderror" rows="3" placeholder="Describe the fault or issue in detail...">{{ old('issue_description', $repair->issue_description) }}</textarea>
                            ...
                            <div class="row g-3 mb-3">
                                ...
                                <div class="col-md-4">
                                    ...
                                    <div class="input-group">
                                        ...
                                        <input type="datetime-local" name="reported_date" class="form-control @error('reported_date') is-invalid @enderror" value="{{ old('reported_date', $repair->reported_date ? $repair->reported_date->format('Y-m-d\TH:i') : '') }}">
                                        @error('reported_date')
                                            <div class="invalid-feedback">{{ $message }}</div>
                                        @enderror
                                    </div>
                                ...
                                <div class="col-md-4">
                                    ...
                                    <div class="input-group">
                                        ...
                                        <input type="datetime-local" name="sent_date" class="form-control @error('sent_date') is-invalid @enderror" value="{{ old('sent_date', $repair->sent_date ? $repair->sent_date->format('Y-m-d\TH:i') : '') }}">
                                        @error('sent_date')
                                            <div class="invalid-feedback">{{ $message }}</div>
                                        @enderror
                                    </div>
                                ...
                                <div class="col-md-4">
                                    ...
                                    <div class="input-group">
                                        ...
                                        <input type="datetime-local" name="received_date" class="form-control @error('received_date') is-invalid @enderror" value="{{ old('received_date', $repair->received_date ? $repair->received_date->format('Y-m-d\TH:i') : '') }}">
                                        @error('received_date')
                                            <div class="invalid-feedback">{{ $message }}</div>
                                        @enderror
                                    </div>
                                    <div class="form-text text-muted small">Fill when repairs are finished.</div>
                                </div>
                            </div>
                            ...
                            <div class="row g-3 mb-4">
                                ...
                                <div class="col-md-6">
                                    ...
                                    <div class="input-group">
                                        ...
                                        <input type="number" step="0.01" name="repair_cost" class="form-control @error('repair_cost') is-invalid @enderror" placeholder="0.00" value="{{ old('repair_cost', $repair->repair_cost) }}">
                                        @error('repair_cost')
                                            <div class="invalid-feedback">{{ $message }}</div>
                                        @enderror
                                    </div>
                                ...
                                <div class="col-md-6">
                                    ...
                                    <div class="input-group">
                                        ...
                                        <select name="repair_status" class="form-select @error('repair_status') is-invalid @enderror" required>
                                            <option value="Reported" {{ old('repair_status', $repair->repair_status) == 'Reported' ? 'selected' : '' }}>Reported</option>
                                            <option value="Sent for Repair" {{ old('repair_status', $repair->repair_status) == 'Sent for Repair' ? 'selected' : '' }}>Sent for Repair</option>
                                            <option value="Under Repair" {{ old('repair_status', $repair->repair_status) == 'Under Repair' ? 'selected' : '' }}>Under Repair</option>
                                            <option value="Repaired" {{ old('repair_status', $repair->repair_status) == 'Repaired' ? 'selected' : '' }}>Repaired</option>
                                            <option value="Received" {{ old('repair_status', $repair->repair_status) == 'Received' ? 'selected' : '' }}>Received</option>
                                            <option value="Unrepairable" {{ old('repair_status', $repair->repair_status) == 'Unrepairable' ? 'selected' : '' }}>Unrepairable</option>
                                            <option value="Disposed" {{ old('repair_status', $repair->repair_status) == 'Disposed' ? 'selected' : '' }}>Disposed</option>
                                        </select>
                                        @error('repair_status')
                                            <div class="invalid-feedback">{{ $message }}</div>
                                        @enderror
                            ...
                            <div class="mb-4">
                                ...
                                <textarea name="remarks" class="form-control @error('remarks') is-invalid @enderror" rows="2" placeholder="Enter any notes...">{{ old('remarks', $repair->remarks) }}</textarea>
                                @error('remarks')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                            </div>
                            ...
                            <div class="d-flex justify-content-end gap-2 border-top pt-3">
                                ...
                                <button type="submit" class="btn btn-warning px-4 text-dark fw-semibold">
                                    <i class="fa fa-save me-1"></i> Update Request
                                </button>
                            </div>
